...
public class VSNController : MonoBehaviour {
  ...
  public void StartVSNScript(string scriptToLoad, string waypointToStart){
    StartVSNScript(scriptToLoad, 0);
    int lineToStart = VSNCommands.GetInstance().waypoints[waypointToStart];
    VSNScriptReader.GetInstance().GoToLine(lineToStart);
  }
  ...
  public void StartVSNScript(string scriptToLoad, int lineToLoad){
    ...
    reader.LoadScript();
    VSNScriptReader.GetInstance().GoToLine(lineToLoad);
  }

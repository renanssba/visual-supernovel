...
public class CharacterList : MonoBehaviour {
  ...
  public int GetCharIdByParam(string param){
	...
	public void ExecuteAnimateAlpha(int charIndex, float anim_time, float end_alpha){
		if(anim_time == 0){
      characters[charIndex].SetAlpha(end_alpha); // change alpha quickly
    }else{
      characters [charIndex].gameObject.GetComponent<CanvasGroup> ().DOFade (end_alpha, anim_time);
      //StartCoroutine(AnimateAlpha(characters[charIndex], anim_time, end_alpha));
		}
	}

  IEnumerator AnimateAlpha(Character character, float anim_time, float end_alpha){
    float start_alpha = character.GetAlpha();
		float elapsed_time = 0;
				
    while(elapsed_time < anim_time){
      character.SetAlpha( Mathf.Lerp(start_alpha, end_alpha,  Mathf.SmoothStep(0.0f, 1.0f, elapsed_time / anim_time)) );
			elapsed_time += Time.deltaTime;
			yield return new WaitForEndOfFrame();
		}
    character.SetAlpha(end_alpha);
	}

	public void AnimateScale(int charIndex, float anim_time, float scale_y){

		Transform character = characters[charIndex].transform;
    Image charImage = character.GetComponent<Image>();
		Vector2 currentScale = charImage.rectTransform.sizeDelta;
		float scale_x = currentScale.x +(Mathf.Abs(scale_y - currentScale.y));
		Vector2 newScale = new Vector2(scale_x, scale_y);

		if(anim_time == 0)
			charImage.rectTransform.sizeDelta = newScale; // scale quickly
		else{
      IEnumerator scale = Scale(character, anim_time, currentScale, newScale);
	...
	IEnumerator Scale(Transform character, float anim_time, Vector2 startScale, Vector2 endScale){
		...
		while(elapsedTime < time){
			charImage.rectTransform.sizeDelta = Vector2.Lerp(startScale, endScale,  Mathf.SmoothStep(0.0f, 1.0f, elapsedTime / time));
			elapsedTime += Time.deltaTime;
			yield return new WaitForEndOfFrame();
		}
		charImage.rectTransform.sizeDelta = endScale;
	}
